...
export const PUT = withAdminAuth("online.manage", async (req, ctx) => {
  ...
  const { shop_domain, fulfillment_location_id, access_token, webhook_secret, shopify_location_id } = v.data;
    ...
    await client.query(
      `INSERT INTO channel_integrations
         (organization_id, provider, shop_domain, fulfillment_location_id, shopify_location_id,
          ...
          status, updated_at)
       VALUES ($1, 'shopify', $2, $3, $4, $5, $6, $7, 'disconnected', now())
       ON CONFLICT (organization_id, provider) DO UPDATE SET
         ...
         updated_at = now()`,
      [orgId, shop_domain, fulfillment_location_id, shopify_location_id ?? null, tokenCipher, secretCipher, employee.id],
    );
    await client.query(
